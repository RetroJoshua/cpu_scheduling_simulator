...
export function simulateSJF(processes: Process[]): SimulationResult {
  ...
  while (remainingProcesses.length > 0) {
    ...
    if (availableProcesses.length === 0) {
      ...
      steps.push({
        time: currentTime,
        action: 'CPU Idle (waiting for process arrival)',
        processName: 'Idle',
        readyQueue: [],
    ...
    steps.push({
      time: currentTime,
      action: `Start executing ${process.name} (Burst: ${process.burstTime})`,
      processName: process.name,
      readyQueue: availableProcesses.slice(1).map(p => p.name),
      ganttItems: [...ganttChart]
    });
    ...
    steps.push({
      ...
      processName: process.name,
      readyQueue: remainingProcesses.filter(p => p.arrivalTime <= currentTime).map(p => p.name),
      ganttItems: [...ganttChart]
    });
  }
